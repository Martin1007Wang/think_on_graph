...
class GraphConstrainedPromptBuilder(object):
    ...
    def get_graph_index(self, question_dict):
        ...
        if "paths" in question_dict:
            ...
            if self.add_rule and question_dict.get('predicted_paths'):
                paths_list = self.apply_rules(g, question_dict['predicted_paths'], question_dict["q_entity"])
            else:
    ...
    def process_input(self, question_dict, return_tire=True):
        # 提取基本信息
        question = question_dict["question"] if question_dict["question"].endswith("?") else question_dict["question"] + "?"
        start_node = question_dict["q_entity"]
        answer_node = question_dict["a_entity"]
        choices = question_dict.get("choices", [])

        # 获取trie索引
        trie = self.get_graph_index(question_dict) if return_tire else None

        # 获取真实路径
        g = utils.build_graph(question_dict["graph"], self.undirected)
        truth_paths = utils.get_truth_paths(start_node, answer_node, g)
        ...
        return input_text, ground_paths, trie
...
class RetrievalPromptBuilder(GraphConstrainedPromptBuilder):
    ...
    def get_graph_index(self, question_dict):
        # 获取或生成路径
        cache_paths_list = question_dict.get("paths")
        g = utils.build_graph(question_dict["graph"], self.undirected)
        
        if self.add_rule and question_dict.get('predicted_paths'):
            paths_list = self.apply_rules(g, question_dict['predicted_paths'], question_dict["q_entity"])
        else:
            paths_list = cache_paths_list or utils.dfs(g, question_dict["q_entity"], self.index_path_length)
        ...
        # 构建trie索引
        entity_trie = MarisaTrie([
            self.tokenizer(e, add_special_tokens=False).input_ids + [self.tokenizer.eos_token_id] 
            for e in entity_list
        ])
        
        relation_trie = MarisaTrie([
            self.tokenizer(r, add_special_tokens=False).input_ids + [self.tokenizer.eos_token_id]
            for r in relation_list
        ])
        
        triple_trie = MarisaTrie([
            self.tokenizer(f"[{h}, {rel}, {t}]", add_special_tokens=False).input_ids + [self.tokenizer.eos_token_id]
    ...
    def process_input(self, question_dict, return_tire=True):
        ...
        if return_tire:
            ...
            trie = entity_list = relation_list = triple_list = paths_list = None
