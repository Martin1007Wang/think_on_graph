...
SFT_DATASET_OUTPUT_BASE="${BASE_WORKDIR}/data/instruction_dataset" # Changed for SFT

# Base name for the SFT dataset outputs (suffixes will be added by Python script)
SFT_BASE_OUTPUT_NAME="${DATASET_NAME}_${SPLIT}_sft_instruct" # Changed for SFT

# Python script names (assuming they are in a 'workflow' subdirectory)
PREPARE_PATHS_SCRIPT="workflow/prepare_paths.py"
CREATE_SFT_SCRIPT="workflow/create_chat_dataset.py" 
