...
def create_instruction_example_no_ids(question: str, entity: str, available_relations: List[str], 
                                     chosen_relations: List[str], max_k_relations: int) -> Dict:
    """Create an instruction example for relation selection without using REL_IDs."""
    # Create a simple bullet list of relations
...
Your response should ONLY contain the exact relation names from the list above.
Your selection (exact relation names only, up to {max_k_relations}):"""
    
    # Create completion using actual relation names (not IDs)
    # Limit to max_k_relations
    chosen_rels = chosen_relations[:max_k_relations]
    completion = ", ".join(chosen_rels) if chosen_rels else "None of these relations seem relevant"
...
def process_path_item(item: Dict, config: ProcessingConfig) -> List[Dict]:
    ...
    for hop, src_relations in positive_hop_relations.items():
        for src_entity, pos_relations in src_relations.items():
            ...
            # Create instruction example without IDs
            example = create_instruction_example_no_ids(
                question=question,
                entity=src_entity,
...
def create_chat_dataset_no_ids(args):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create SFT instruction dataset from path data without using REL_IDs")
    ...
    parser.add_argument('--output_name', type=str, default='relation_sft_dataset_no_ids', 
    ...
    create_chat_dataset_no_ids(args) 