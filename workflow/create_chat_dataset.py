...
def create_direct_instruction_example(question: str, entity: str, relation: str) -> Dict:
    """Create an instruction example for direct relation prediction."""
    
    prompt = f"""You are a knowledge graph expert. Based on the question and entity, predict the most relevant relation to query.
...
What relation from this entity would be most useful to explore to answer the question? 
Provide only the relation name, no additional text."""
...
def process_path_item(item: Dict) -> List[Dict]:
    ...
    for hop, src_relations in positive_hop_relations.items():
        for src_entity, pos_relations in src_relations.items():
            # Create individual examples for each positive relation
            for relation in pos_relations:
                # Create instruction example
                example = create_direct_instruction_example(
                    question=question,
                    entity=src_entity,
                    relation=relation
                )
                ...
                # Add metadata
                example["metadata"] = {
                    "id": f"{sample_id}_hop{hop}_{src_entity}_{relation}",
                    "hop": hop,
...
def create_chat_dataset(args):
    ...
    try:
        for item in tqdm(path_data, desc="Processing path items"):
            instruction_examples = process_path_item(item)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path', type=str, default="/mnt/wangjingxiong/think_on_graph/data/data.json", 
    ...
    parser.add_argument('--output_name', type=str, default='RoG-webqsp_train_sft_dataset_single', 
